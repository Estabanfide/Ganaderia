...
namespace SistemaGestiónGanaderaLaMestiza.Repositorios
{
    public class DashboardRepository
    {
        public DashboardMetricas GetMetricas()
        {
            ...
            const string vacasSql = "SELECT COUNT(*) AS C FROM animales WHERE sexo = 'F' AND MONTHS_BETWEEN(SYSDATE, fecha_nac) / 12 >= 1";
            ...
            const string ternerasSql = "SELECT COUNT(*) AS C FROM animales WHERE sexo = 'F' AND MONTHS_BETWEEN(SYSDATE, fecha_nac) / 12 < 1";
