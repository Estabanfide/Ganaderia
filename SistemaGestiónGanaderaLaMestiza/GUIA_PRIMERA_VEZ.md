...
## 4. Paso 2: Columna "clave" en la tabla usuarios (login/registro)

Para que el registro y el login funcionen:

1. Abre **SQL*Plus**, **SQL Developer** o tu herramienta Oracle.
2. Conéctate con el mismo usuario que configuraste en Web.config.
3. Ejecuta el script **Scripts/scripts/add-clave-usuarios.sql** (o ejecuta manualmente: `ALTER TABLE usuarios ADD clave VARCHAR2(255);`).
4. A partir de ahí la aplicación podrá guardar contraseñas al registrarse y comprobarlas al iniciar sesión.

Si no ejecutas este script, el registro fallará.
...
| Error al registrarse / "invalid identifier" en clave | Ejecuta el script **add-clave-usuarios.sql** en Oracle para añadir la columna **clave** a la tabla **usuarios**. |
